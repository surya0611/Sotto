...
export async function POST(request: NextRequest) {
  try {
    ...
    const bigcommerce_secret = account.integration_secrets?.bigcommerce_secret;
    if (!bigcommerce_secret) {
      return NextResponse.json({ error: 'BigCommerce integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-bigcommerce-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== bigcommerce_secret) {
    ...
    if (insertError) {
      console.error('Error inserting BigCommerce event:', insertError);
