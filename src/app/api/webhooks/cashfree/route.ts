...
export async function POST(request: NextRequest) {
  try {
    ...
    const secretProvided = request.headers.get('x-cashfree-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== cashfree_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Cashfree event:', insertError);
