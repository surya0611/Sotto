...
export async function POST(request: NextRequest) {
  try {
    ...
    const dukaan_secret = account.integration_secrets?.dukaan_secret;
    if (!dukaan_secret) {
      return NextResponse.json({ error: 'Dukaan integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-dukaan-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== dukaan_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Dukaan event:', insertError);
      return NextResponse.json({ error: 'Failed to insert event' }, { status: 500 });
    }

    return new NextResponse('OK', { status: 200 });
  } catch (error) {
    console.error('Dukaan webhook error:', error);
