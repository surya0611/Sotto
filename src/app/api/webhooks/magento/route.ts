...
export async function POST(request: NextRequest) {
  try {
    ...
    const magento_secret = account.integration_secrets?.magento_secret;
    if (!magento_secret) {
      return NextResponse.json({ error: 'Magento integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-magento-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== magento_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Magento event:', insertError);
      return NextResponse.json({ error: 'Failed to insert event' }, { status: 500 });
    }

    return new NextResponse('OK', { status: 200 });
  } catch (error) {
    console.error('Magento webhook error:', error);
