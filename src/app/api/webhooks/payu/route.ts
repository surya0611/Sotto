...
export async function POST(request: NextRequest) {
  try {
    ...
    const payu_secret = account.integration_secrets?.payu_secret;
    if (!payu_secret) {
      return NextResponse.json({ error: 'PayU integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-payu-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== payu_secret) {
    ...
    if (insertError) {
      console.error('Error inserting PayU event:', insertError);
