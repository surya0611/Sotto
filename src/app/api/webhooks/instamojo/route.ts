...
export async function POST(request: NextRequest) {
  try {
    ...
    const instamojo_secret = account.integration_secrets?.instamojo_secret;
    if (!instamojo_secret) {
      return NextResponse.json({ error: 'Instamojo integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-instamojo-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== instamojo_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Instamojo event:', insertError);
