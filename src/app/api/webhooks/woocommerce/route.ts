...
export async function POST(request: NextRequest) {
  try {
    ...
    const woocommerce_secret = account.integration_secrets?.woocommerce_secret;
    if (!woocommerce_secret) {
      return NextResponse.json({ error: 'WooCommerce integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-woocommerce-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== woocommerce_secret) {
    ...
    if (insertError) {
      console.error('Error inserting WooCommerce event:', insertError);
