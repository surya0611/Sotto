...
export async function POST(request: NextRequest) {
  try {
    ...
    const jumpseller_secret = account.integration_secrets?.jumpseller_secret;
    if (!jumpseller_secret) {
      return NextResponse.json({ error: 'Jumpseller integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-jumpseller-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== jumpseller_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Jumpseller event:', insertError);
      return NextResponse.json({ error: 'Failed to insert event' }, { status: 500 });
    }

    return new NextResponse('OK', { status: 200 });
  } catch (error) {
    console.error('Jumpseller webhook error:', error);
