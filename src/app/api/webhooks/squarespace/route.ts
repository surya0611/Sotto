...
export async function POST(request: NextRequest) {
  try {
    ...
    const squarespace_secret = account.integration_secrets?.squarespace_secret;
    if (!squarespace_secret) {
      return NextResponse.json({ error: 'Squarespace integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-squarespace-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== squarespace_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Squarespace event:', insertError);
