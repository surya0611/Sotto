...
export async function POST(request: NextRequest) {
  try {
    ...
    const ecwid_secret = account.integration_secrets?.ecwid_secret;
    if (!ecwid_secret) {
      return NextResponse.json({ error: 'Ecwid integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-ecwid-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== ecwid_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Ecwid event:', insertError);
      return NextResponse.json({ error: 'Failed to insert event' }, { status: 500 });
    }

    return new NextResponse('OK', { status: 200 });
  } catch (error) {
    console.error('Ecwid webhook error:', error);
