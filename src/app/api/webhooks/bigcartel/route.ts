...
export async function POST(request: NextRequest) {
  try {
    ...
    const bigcartel_secret = account.integration_secrets?.bigcartel_secret;
    if (!bigcartel_secret) {
      return NextResponse.json({ error: 'Big Cartel integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-bigcartel-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== bigcartel_secret) {
    ...
    if (insertError) {
      console.error('Error inserting Big Cartel event:', insertError);
      return NextResponse.json({ error: 'Failed to insert event' }, { status: 500 });
    }

    return new NextResponse('OK', { status: 200 });
  } catch (error) {
    console.error('Big Cartel webhook error:', error);
