...
export async function POST(request: NextRequest) {
  try {
    ...
    const lightspeed_secret = account.integration_secrets?.lightspeed_secret;
    if (!lightspeed_secret) {
      return NextResponse.json({ error: 'LightSpeed integration not configured' }, { status: 401 });
    }

    const secretProvided = request.headers.get('x-lightspeed-secret') || searchParams.get('secret');
    if (secretProvided && secretProvided !== lightspeed_secret) {
    ...
    if (insertError) {
      console.error('Error inserting LightSpeed event:', insertError);
