...
export function AppearanceForm({ initialTheme }: { initialTheme: any }) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setTheme(prev => ({ ...prev, [e.target.name]: value }));
  };
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-5)' }}>
            ...
            <div style={{ background: 'var(--bg-muted)', padding: 'var(--s-6)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)' }}>
              ...
              <div className="input-group">
                ...
                <select name="hover_animation" className="input" value={theme.hover_animation} onChange={handleChange}>
                  {HOVER_ANIMATIONS.map(a => <option key={a.id} value={a.id}>{a.label}</option>)}
                </select>
              </div>
              <div className="input-group" style={{ marginTop: '16px' }}>
                <label className="input-label">Slide Animation</label>
                <select name="slide_animation" className="input" value={theme.slide_animation} onChange={handleChange}>
...
console.log('Done rewriting appearance-form.tsx');
